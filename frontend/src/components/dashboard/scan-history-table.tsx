...
import { ArrowUpRight, BadgeCheck, Rocket, ShieldCheck } from "lucide-react";
        ...
        {scans.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-cyan-400/25 bg-slate-950/40 p-8 text-center">
            ...
            <div className="mt-6 grid gap-3 md:grid-cols-3">
              {[
                { icon: Rocket, title: "1. Upload a QR image", text: "Drag and drop a screenshot, receipt, label, or payment QR." },
                { icon: BadgeCheck, title: "2. Review the verdict", text: "Check the severity badge, risk score, and confidence on the result page." },
                { icon: ShieldCheck, title: "3. Keep an audit trail", text: "Store the scan to compare future threats and community reports." }
              ].map((step) => {
                const Icon = step.icon;
                return (
                  <div key={step.title} className="rounded-xl border border-border bg-slate-950/50 p-4 text-left">
              ...
              <TableBody>
                {scans.map((scan) => (
                  <TableRow key={scan.id} className="hover:bg-cyan-400/5">
                    ...
                    <TableCell>
                      <Badge variant={Math.min(99, Math.max(82, 100 - Math.round(scan.riskScore / 5))) >= 92 ? "success" : "default"}>
                        {Math.min(99, Math.max(82, 100 - Math.round(scan.riskScore / 5)))}%
                      </Badge>
                    </TableCell>
                    <TableCell>
                      <SeverityBadge severity={scan.severity} />
                    </TableCell>
                    <TableCell className="text-slate-300">{new Date(scan.createdAt).toLocaleString()}</TableCell>
                    <TableCell>
